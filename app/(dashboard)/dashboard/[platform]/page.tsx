...
import { platformFromSlug, platformConfigs } from "@/services/bot-platforms/config";
...
export default async function DashboardPage({
  ...
  const [botsCount, activeCampaignsCount, postsCount] = await Promise.all([
    ...
    prisma.campaign.count({
      where: {
        isActive: true,
        bot: { userId: session.userId, platform: platformValue },
      },
    }),
    prisma.post.count({
      where: { bot: { userId: session.userId, platform: platformValue } },
    }),
  ]);
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            نمای سریع از فعالیت ربات‌های {platformConfigs[platformSlug].labelFa}،
            محتوا و کمپین‌های شما
          </p>
        </div>
      </div>
