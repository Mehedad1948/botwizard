...
// 2. Send Now
    ...
    // دریافت گروه‌های ربات از دیتابیس
    ...
    if (connectedGroups.length === 0) {
        await callTelegramAPI("editMessageText", {
            ...
            text: "❌ ربات شما هنوز در هیچ گروه یا کانالی عضو نشده است.\nابتدا ربات را به گروه خود اضافه کرده و دسترسی ادمین بدهید."
        }, botToken);
        return;
    }

    const keyboard = connectedGroups.map(group => (
        [{ text: `📢 ${group.chatTitle}`, callback_data: `sn_exec_${draftId}_${group.chatId}` }]
    ));
...
// 3. STEP ONE: Schedule Clicked -> Show Target Groups
    ...
    const keyboard = connectedGroups.map(group => (
        [{ text: `📢 ${group.chatTitle}`, callback_data: `sg_${draftId}_${group.chatId}` }]
    ));
    keyboard.push([{ text: "🔙 بازگشت", callback_data: "cancel_draft" }]);
