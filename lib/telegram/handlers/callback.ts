...
export async function handleCallbackQuery(callback_query: any, bot: Bot) {
    ...
    if (data === "menu_help") {
        const helpText = `📖 <b>راهنمای ارسال و زمان‌بندی محتوا</b>

این ربات برای انتشار محتوا در گروه‌ها و کانال‌های متصل‌شده به شما استفاده می‌شود.

<b>۱. اتصال مقصدها</b>
ربات را به گروه یا کانال موردنظر اضافه کنید و دسترسی ادمین، به‌ویژه مجوز ارسال پیام، را به آن بدهید. پس از اتصال موفق، پیام تأیید دریافت می‌کنید.

<b>۲. ایجاد پست</b>
روی «📝 ایجاد پست جدید» بزنید و سپس متن، عکس یا ویدیوی موردنظر را در همین گفت‌وگو ارسال کنید.

<b>۳. انتخاب مقصد</b>
پس از دریافت محتوا، «ارسال فوری» یا «زمان‌بندی» را انتخاب کنید؛ سپس یک، چند یا همه مقصدهای متصل را علامت بزنید.

<b>۴. انتخاب زمان ارسال</b>
برای زمان‌بندی می‌توانید یکی از این روش‌ها را انتخاب کنید:
• <b>دوره‌ای:</b> ارسال مجدد پس از یک فاصله زمانی ثابت
• <b>ساعت مشخص:</b> ارسال روزانه در ساعت‌هایی مانند <code>09:00</code> یا <code>18:30</code>

تمام ساعت‌های مشخص براساس <b>زمان ایران</b> (<code>Asia/Tehran</code>) محاسبه می‌شوند.

<b>۵. مدیریت ارسال‌ها</b>
از «📊 مدیریت کمپین‌ها» برای مشاهده، توقف، فعال‌سازی یا حذف زمان‌بندی‌ها استفاده کنید. در «👥 مدیریت گروه‌ها» نیز می‌توانید مقصدهای متصل را مشاهده یا ربات را از آن‌ها خارج کنید.

⚠️ برای انجام ارسال‌ها، ربات باید فعال بماند و دسترسی ارسال پیام در مقصد را حفظ کند.`;
